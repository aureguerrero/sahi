...
#             mask[np.where(mask>0)]=objeto.category.id+1
    ...
    def lineas(self, fft_threshold=0.93,nminppl=10,clear =None, cota_transf=0.2,cambio=1):
        ...
        dist_media_rec=np.mean([0.5*np.abs(rectas[i].coef[1]-rectas[i+1].coef[1])/np.mean([np.sqrt(rectas[r].coef[0]**2+1) for r in range(len(rectas))]) for i in range(len(rectas)-1)])
        lineas_d_surcos=[]
        ...
        for i in range(len(rectas)-1):
          ...
          if len(ubic)>=nminppl:
            ...
            if rectas[0].coef[0]<0:
              media_altura=np.mean(np.array([0.5*np.max(self.object_prediction_list[l].mask.shape[1:2])/np.cos(np.arctan(rectas[0].coef[0])+np.pi/2) for l in ubic]))
            else:
              media_altura=np.mean(np.array([0.5*np.max(self.object_prediction_list[l].mask.shape[1:2])/np.sin(np.arctan(rectas[0].coef[0])+np.pi/2) for l in ubic]))
...
#           self.centroides=[self.centroides[i] for i in range(len(self.centroides)) if i not in set(ptos_a_sacar)]
        if len(ptos_a_sacar)>0 and cambio==1:
            ...
            for r in range(len(info_d_surcos)):
              ...
              datos=centros[info_d_surcos[r]['ubic'],:]
              if rectas[0].coef[0]<0:
                media_altura=np.mean(0.5*np.array([np.max(self.object_prediction_list[l].mask.shape[1:2])/np.cos(np.arctan(rectas[0].coef[0])+np.pi/2) for l in info_d_surcos[r]['ubic']]))
              else:
                media_altura=np.mean(0.5*np.array([np.max(self.object_prediction_list[l].mask.shape[1:2])/np.sin(np.arctan(rectas[0].coef[0])+np.pi/2) for l in info_d_surcos[r]['ubic']]))
              media_altura=np.min([media_altura,dist_media_rec])
